...
Library API for escape hatches: [references/document-creation.md § openpyxl](../document-creation.md#1-openpyxl-excel-xlsx).
...
- **When `claw xlsx` isn't enough** — [openpyxl escape hatches](#when-claw-isnt-enough)
...
Drop into `openpyxl` directly for these scenarios — see [document-creation.md § openpyxl](../document-creation.md#1-openpyxl-excel-xlsx).

| Use case | Why `claw` can't do it | Library anchor |
|---|---|---|
| Arbitrary chart construction (combo charts, dual axes, custom series shapes) | Covered flags can't express every chart topology | [Charts](../document-creation.md#16-charts) |
| Overlapping conditional-formatting rule stacks with explicit `stopIfTrue` ordering | Ranges intersect; order matters | [Conditional formatting](../document-creation.md#14-conditional-formatting) |
| VBA-preserving round-trip (`keep_vba=True`) | No VBA authoring surface | [Workbook operations](../document-creation.md#11-workbook--worksheet-operations) |
| Pivot-table creation (only read / preserve is supported) | `openpyxl` doesn't author pivots either — use a template `.xlsx` | — |
| Worksheet-scoped named styles with inheritance chains | Flag surface can't represent style hierarchies | [Styling & formatting](../document-creation.md#13-styling--formatting) |
