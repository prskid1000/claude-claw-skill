...
```bash
claw xlsx richtext <SRC_XLSX> ...
```

### 3.3 `image`
Image operations on a workbook.
```bash
claw xlsx image <SRC_XLSX> ...
```
...
```bash
claw xlsx name add <SRC_XLSX> --name <NAME> --refers-to <REF> [--force]
```

### 5.3 `print-setup`
Set print area, repeat titles, fit-to-page, orientation, paper size.
```bash
claw xlsx print-setup <SRC_XLSX> --sheet <NAME> [--print-area RANGE] [--force]
```
...
```bash
claw xlsx meta <get|set> <SRC_XLSX> ...
```

### 7.2 `pivots`
Pivot-table operations (read-only).
```bash
claw xlsx pivots list <SRC_XLSX> [--json]
```
